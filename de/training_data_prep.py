...
class LossPrevTrainingDataPrep(object):
    ...
    """
        ...
        self.da_features_select_fname = "da_select_" + os.path.basename(
            self.redshift_trans_csv_fname
        )
        self.ml_features_select_fname = "ml_select_" + os.path.basename(
            self.redshift_trans_csv_fname
        )
    ...
    """
        ...
        fname = os.path.join(self.output_path, self.ml_features_select_fname)

        select_df = pd.read_csv(
            fname, skip_blank_lines=True, warn_bad_lines=True, error_bad_lines=False
        )
        ...
        fname = os.path.join(
            self.output_path,
            "ml_select_with_label_" + os.path.basename(self.redshift_trans_csv_fname),
        )
    ...
    """
        ...
        fname = os.path.join(self.output_path, self.da_features_select_fname)
        ...
        paycode_logical_or = np.logical_or(
            np.logical_or(np.logical_or(paycode == 3, paycode == 4), paycode == 6),
            paycode == 10,
        )
      ...
      """
            ...
            car_tup = (ticketid_tup, (row["make"], row["color"], row["plate"]))
        ...
        fname = os.path.join(
            self.output_path,
            "da_select_with_label_" + os.path.basename(self.redshift_trans_csv_fname),
        )
        select_df.to_csv(fname)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-da_f",
        "--data_aug_features_columns_fname",
        type=str,
        required=True,
        help="Text file to have ML features' headers",
    )
    parser.add_argument(
        "-o", "--output_dir", type=str, required=True, help="Output dir"
    )
    ...
    trn_dataprep = LossPrevTrainingDataPrep(
        redshift_trans_csv_fname,
        ml_features_columns_fname,
        da_features_columns_fname,
        output_dir,
